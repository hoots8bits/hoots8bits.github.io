...
export class Stage5 extends Phaser.Scene{
    ...
    create(){
        ...
        this.stagetrack = this.sound.add("discEncounter")
        this.stagetrack.setVolume(0.8);
        this.stagetrack.play()
        this.stagetrack.loop
        ...
        */
        ...
        this.stage5Tilemap = this.add.tilemap('stage5Tilemap');

        const tileSet = this.stage5Tilemap.addTilesetImage("tileset_stage5", "tileset_stage5");
        


        const spawnPoint = this.stage5Tilemap.findObject("playerSpawn", obj => obj.name === "playerSpawn")

        

        const BCLayer = this.stage5Tilemap.createLayer("BackgroundDeco", tileSet)
        this.breakableWallLayer = this.stage5Tilemap.createLayer("breakableWall", tileSet)
        const spikesLayer = this.stage5Tilemap.createLayer("spike", tileSet);
        const floorLayer = this.stage5Tilemap.createLayer("platforms", tileSet)
        this.ladderLayer = this.stage5Tilemap.createLayer("ladder", tileSet)
        this.wobblePlatLayer = this.stage5Tilemap.createLayer("wobblePlat", tileSet)
        ...
        this.physics.add.overlap(this.player, this.ladderLayer, (body, tile)=>{
            if(tile.index === 9 || tile.index === 12){
        ...
        const boss = this.stage5Tilemap.findObject("boss", obj => obj.name === "boss")
    ...
    update(/*delta*/){
        ...
        this.registry.set("discsCollected", 4)
        //boss
        ...
        const camLock1 = this.stage5Tilemap.findObject("cameraLock", obj => obj.name === "camStart")
        const camLock2 = this.stage5Tilemap.findObject("camlock2", obj => obj.name === "camFinish")
    ...
    victory(){
        if(!this.onVictory){
            this.onVictory = true
            alert("I'm tired.")
            alert("I have been since a long time.")
            alert("I am tired of helping out others, it's damaging myself.")
            alert("Thanks for making the time to listen to the discs. They have been recorded but not listened whatsoever.")
            alert("")
            alert("")
            alert("Please be careful, and apreciate the real, silly things of the life")
            alert("Because, even if you have gone trough all, your fate will be waiting for you to catch up.")
