...
export class Stage4 extends Phaser.Scene{
    ...
    init(){
        ...
        this.wallCount = 0
        this.backgroundLooping = false
        this.wallInvert = false
    }
    create(){
        ...
        this.stagetrack = this.sound.add("stage4")
        this.stagetrack.setVolume(0.6);
        this.stagetrack.play()
        this.stagetrack.addMarker({
            name: 'loopPoint',
            start: 22.41, // Start playing from 5 seconds
            config: {
                loop: true // Automatically loop within the marker
            }
        });
        this.stagetrack.on('complete', () => {
            this.stagetrack.play('loopPoint'); // Play again
            this.stagetrack.setVolume(0.6);
        });
        /*
        const controlConfig = {
        ...
        */
        ...
        this.stage4Tilemap = this.add.tilemap('stage4Tilemap');

        const tileSet = this.stage4Tilemap.addTilesetImage("tileset_stage4", "tileset_stage4");
        


        const spawnPoint = this.stage4Tilemap.findObject("playerSpawn", obj => obj.name === "playerSpawn")

        

        const BCLayer = this.stage4Tilemap.createLayer("BackgroundDeco", tileSet)
        this.breakableWallLayer = this.stage4Tilemap.createLayer("breakableWall", tileSet)
        const spikesLayer = this.stage4Tilemap.createLayer("spike", tileSet);
        const floorLayer = this.stage4Tilemap.createLayer("platforms", tileSet)
        this.ladderLayer = this.stage4Tilemap.createLayer("ladder", tileSet)
        this.wobblePlatLayer = this.stage4Tilemap.createLayer("wobblePlat", tileSet)
        ...
        //Disc stuff
        let discWasColected = this.registry.get('disc4Collected')
        ...
        if(this.discAppear){
            ...
            const discStageXY = this.stage4Tilemap.findObject("disc", obj => obj.name === "disc")
            this.disc = this.discSpawn.create(discStageXY.x, discStageXY.y, "disc4")
        ...
        const boss = this.stage4Tilemap.findObject("boss", obj => obj.name === "boss")
        ...
        this.floodGroup = this.physics.add.group()

        this.flood = this.floodGroup.create(-126, 112, "flood")
        this.flood.setSize(1, 288)
        this.flood.setOffset(256, 0)
    ...
    update(/*delta*/){
        ...
        this.cameraView = this.cameras.main.worldView
        if(!this.gameOver){
            if(!this.cameraView.contains(this.flood.x + 128, this.flood.y)){
                this.time.delayedCall(500, ()=>{
                    if(!this.cameraView.contains(this.flood.x + 128, this.flood.y))
                        this.flood.setVelocityX(400)
                })
            }
            else if(this.cameraView.contains(this.flood.x + 128, this.flood.y)){
                this.flood.setVelocityX(35)
            }
        }
        else if(this.gameOver){
            this.flood.setVelocityX(0)
        }

        //flood wall

        if((this.player.x < this.flood.x + 80) && !this.gameOver){
        ...
        if(!this.wallInvert){
            this.wallInvert = true
            this.flood.setFlipY(true)
            this.time.delayedCall(200, ()=>{
                this.flood.setFlipY(false)
                this.time.delayedCall(200, ()=>{
                    this.wallInvert = false
                })
            })
        ...
        const bossRepos = this.stage4Tilemap.findObject("boss", obj => obj.name === "bossGo")

        if(this.boss.x != bossRepos.x && this.boss.y != bossRepos.y && !this.reposPreventer){
            if(this.cameraView.contains(this.boss.x, this.boss.y)){
                this.time.delayedCall(200, ()=>{
                    this.physics.moveTo(this.boss, bossRepos.x, bossRepos.y, 200)
                })
                    
            }
            else if (!this.cameraView.contains(this.boss.x, this.boss.y)){
                this.time.delayedCall(200, ()=>{
                    if(!this.cameraView.contains(this.boss.x, this.boss.y)){
                        this.boss.setVelocityX(0)
                        this.boss.setVelocityY(0)
                        this.boss.setPosition(bossRepos.x, bossRepos.y)
                        this.reposPreventer = true
                    }
                })
            }
        }
        ...
        if(!this.backgroundLooping){
            this.backgroundLooping = true
            this.time.delayedCall(100, ()=>{
                this.cameras.main.setBackgroundColor('#a23000')
                this.time.delayedCall(200, ()=>{
                    this.cameras.main.setBackgroundColor('#e35100')
                    this.time.delayedCall(100, ()=>{
                        this.cameras.main.setBackgroundColor('#ff7930')
                        this.time.delayedCall(100, ()=>{
                            this.cameras.main.setBackgroundColor('#ffa200')
                            this.time.delayedCall(100, ()=>{
                                this.cameras.main.setBackgroundColor('#ffa200')
                                this.time.delayedCall(100, ()=>{
                                    this.time.delayedCall(100, ()=>{
        ...
        const camLock1 = this.stage4Tilemap.findObject("cameraLock", obj => obj.name === "camStart")
        const camLock2 = this.stage4Tilemap.findObject("camlock2", obj => obj.name === "camFinish")
        const camUnlock = this.stage4Tilemap.findObject("camDisc", obj => obj.name === "discUnlock")
    ...
    victory(){
        if(!this.onVictory){
            this.onVictory = true
            let totalDiscs = this.registry.get("discsCollected")
            if(totalDiscs != 4){
                alert("Enough.")
                alert("Nobility doesn't get you far.")
                const confirmation = confirm("YOU think nobility is the answer?")
                if(confirmation == true){
                    alert("In this society is to step or be stepped. Being noble is useless.")
                }
                else{
                    alert("It'll never be.")
                }
                this.cameras.main.fadeOut(0, 0, 0, 0x000)
                this.gameOverJingle.play()
                this.gameOver = true
                this.gameOverJingle.on("complete", ()=>{
                    this.scene.launch("credits")
                    this.scene.stop("lifeSystem")
                    this.scene.stop("stage4")
                })
            }
            else if(totalDiscs == 4){
    ...
    discInter(player, disc){
        ...
        this.registry.set('disc4Collected', true);
